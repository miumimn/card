...
import OpticianPreview from "@/app/templates-preview/OpticianPreview";
...
export default function OpticianProfilePreviewPage() {
  ...
  useEffect(() => {
    ...
    const fetchRow = async () => {
      ...
      try {
        ...
        const listKeys = ["avatar","photos","frames","lenses","services","other_links"];
        listKeys.forEach(k => {
        ...
        // booking_contact: prefer explicit field, fallback to booking_link or profile_url
        normalized.booking_contact = normalized.booking_contact ?? normalized.booking_link ?? normalized.profile_url ?? "";

        const base = process.env.NEXT_PUBLIC_SUPABASE_URL || "";
        const slug = normalized.slug || "optician";

        const imageFields = ["avatar","photos"];
        ...
        if (Array.isArray(normalized.photos) && normalized.photos.length) normalized.photos = normalized.photos.slice(0, 6);

        console.log("Optician preview fetched normalized row:", normalized);
        setRow(normalized);
      } catch (err: any) {
        setError(String(err));
      } finally {
  ...
  return <OpticianPreview data={row} showFooter={false} />;
}