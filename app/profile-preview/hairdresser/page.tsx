...
import HairdresserPreview from "@/app/templates-preview/HairdresserPreview";
...
export default function HairdresserProfilePreviewPage() {
  ...
  useEffect(() => {
    ...
    async function resolveImage(base: string, slug: string, field: string, raw: any) {
      ...
      // If it's already an https URL, keep it (don't gate on HEAD)
      if (looksLikeValidHttpUrl(s)) {
        return s;
      }

      // candidate from building public URL
      const candidate = buildPublicUrl(base, slug, field, s);
      try {
        if (await headOk(candidate)) return candidate;
      } catch {}

      // fallback: list storage prefix and try to match by file name
      try {
        ...
        console.warn("hairdresser storage list fallback failed", err);
      }
    ...
    const fetchRow = async () => {
      ...
      try {
        ...
        // collect gallery candidates from many legacy keys
        ...
        // dedupe and filter sentinels
        rawGalleryValues = Array.from(new Set(rawGalleryValues.map((x:any) => String(x).trim()))).filter((x:any) => !!x && !isFalsySentinel(x));

        const base = process.env.NEXT_PUBLIC_SUPABASE_URL || "";
        const slug = normalized.slug || "hairdresser";

        // resolve avatar / heroImage
        if (normalized.avatar || normalized.avatar_url || normalized.profileImage) {
          const av = normalized.avatar ?? normalized.avatar_url ?? normalized.profileImage ?? "";
          if (Array.isArray(av)) {
            const resolved = await Promise.all(av.slice(0,6).map((v:any) => resolveImage(base, slug, "avatar", v)));
            normalized.avatar = (resolved || []).filter(Boolean)[0] || "";
          } else {
            const r = await resolveImage(base, slug, "avatar", av);
            normalized.avatar = r || "";
          }
        }

        if (normalized.heroImage || normalized.hero_image || normalized.banner) {
          const hv = normalized.heroImage ?? normalized.hero_image ?? normalized.banner ?? "";
          if (Array.isArray(hv)) {
            const resolved = await Promise.all(hv.slice(0,6).map((v:any) => resolveImage(base, slug, "heroImage", v)));
            normalized.heroImage = (resolved || []).filter(Boolean)[0] || "";
          } else {
            const r = await resolveImage(base, slug, "heroImage", hv);
            normalized.heroImage = r || "";
          }
        }

        // Resolve gallery list (try to keep https urls even if HEAD not ok)
        ...
        const listKeys = ["services","services_list","contact_cards","other_links","packages","checklist"];
        listKeys.forEach(k => {
        ...
        if (process.env.NODE_ENV !== "production") {
          ...
          console.log("Hairdresser preview normalized row:", normalized);
        }
  ...
  return <HairdresserPreview data={row} showFooter={false} />;
}