...
export default function DJPreviewClient() {
  ...
  useEffect(() => {
    ...
    const normalizeRow = (row: any) => {
      ...
      // include listen_link so we pick up the new CTA field
      const keys = ["name","title","about","genres","heroImage","avatar","mixes","mix_links","mixesImages","mixes_images","gigs","email","phone","agent","contact_cards","instagram","tiktok","twitter","facebook","soundcloud","profile_url","listen_link"];
      keys.forEach((k) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // If listen_link provided in extra_fields, promote it
        if ((!normalized.listen_link || normalized.listen_link === "") && normalized.extra_fields && normalized.extra_fields.listen_link) {
          normalized.listen_link = normalized.extra_fields.listen_link;
        }
        ...
        mixes = mixes.map((m, idx) => {
          let image = m.image || "";
          if (image && typeof image === "string" && !/^https?:\/\//.test(image)) {
            // Try common buckets/fields: prefer mixesImages path (legacy), fallback to mix_i_image if present
            if (Array.isArray(normalized.mixesImages) && normalized.mixesImages[idx]) {
              image = normalized.mixesImages[idx];
            } else {
        ...
        if (Array.isArray(normalized.contact_cards)) {
          normalized.contact_cards = normalized.contact_cards
        ...
        // Normalize listen_link into a single string for preview usage (prefer listen_link, fallback to profile_url)
        // do NOT transform scheme here; DJPreview will compute tel: or https: as needed.
        normalized.listen_link = asString(normalized.listen_link ?? normalized.extra_fields?.listen_link ?? normalized.profile_url ?? "");
