...
import GardenerPreview from "@/app/templates-preview/GardenerPreview";

const STORAGE_BUCKET = "onboarding-uploads";

/** per-segment encode helper and normalization **/
...
export default function GardenerProfilePreviewPage() {
  ...
  useEffect(() => {
    ...
    const resolveImageValue = async (base: string, slug: string, field: string, raw: any) => {
      ...
      // fallback: list objects under slug/field and try to match filename fragments
      try {
        ...
        if (!listRes.error && Array.isArray(listRes.data)) {
          ...
          if (found) {
            ...
            // Support both SDK response shapes: { data: { publicUrl } } and older { publicURL }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // parse lists
        const keys = ["services","tips","portfolio","avatar","heroImage","contact_cards"];
        keys.forEach(k => {
        ...
        const slug = normalized.slug || "gardener";

        // resolve image fields (portfolio, avatar, heroImage) with best-effort
        ...
        console.log("Gardener preview fetched normalized row:", normalized);
        setData(normalized);
      } catch (err: any) {
  ...
  // render the gardener preview using the original styling and HIDE the "Use this template" footer (showFooter=false)
  return <GardenerPreview data={data} showFooter={false} />;
}