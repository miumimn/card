...
import ItSpecialistPreview from "@/app/templates-preview/ItSpecialistPreview";
...
export default function ItSpecialistProfilePreviewPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!listRes.error && Array.isArray(listRes.data)) {
          ...
          if (found) {
            ...
            // Safe extraction: getPublicUrl may return { data: { publicUrl } } or older { publicURL }
    ...
    const fetchRow = async () => {
      ...
      try {
        ...
        // parse lists
        const keys = ["portfolio","avatar","heroImage","services"];
        keys.forEach(k => {
        ...
        const slug = normalized.slug || "it-specialist";
        ...
        for (const field of imageFields) {
          ...
          if (Array.isArray(normalized[field])) {
            const resolved = await Promise.all(normalized[field].slice(0, 9).map((v: any) => resolveImage(base, slug, field, v)));
        ...
        if (Array.isArray(normalized.portfolio) && normalized.portfolio.length) normalized.portfolio = normalized.portfolio.slice(0, 9);

        console.log("It-specialist preview fetched normalized row:", normalized);
        setRow(normalized);
      } catch (err: any) {
        setError(String(err));
      } finally {
  ...
  // render preview with user data (showFooter=false for profile preview)
  return <ItSpecialistPreview data={row} showFooter={false} />;
}