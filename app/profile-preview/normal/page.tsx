...
import NormalPreview from "@/app/templates-preview/NormalPreview";
...
export default function NormalProfilePreviewPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!listRes.error && Array.isArray(listRes.data)) {
          ...
          if (found) {
            ...
            // Don't destructure — SDK returns different shapes across versions.
            const res = supabase.storage.from(STORAGE_BUCKET).getPublicUrl(path) as any;
            const publicUrl = (res && res.data && (res.data.publicUrl || res.data.publicURL)) || res?.publicURL || res?.publicUrl || "";
            if (publicUrl) {
    ...
    const fetchRow = async () => {
      ...
      try {
        ...
        const keys = ["avatar","heroImage","photos","other_links"];
        keys.forEach(k => {
        ...
        const slug = normalized.slug || "normal";

        const imageFields = ["avatar","heroImage","photos"];
        for (const field of imageFields) {
          if (!normalized[field]) continue;
          if (Array.isArray(normalized[field])) {
            const resolved = await Promise.all(normalized[field].slice(0, 3).map((v: any) => resolveImage(base, slug, field, v)));
            normalized[field] = resolved.filter(Boolean);
          } else if (typeof normalized[field] === "string") {
            normalized[field] = String(await resolveImage(base, slug, field, normalized[field]));
          }
        }

        if (Array.isArray(normalized.photos) && normalized.photos.length) normalized.photos = normalized.photos.slice(0, 3);
        if (Array.isArray(normalized.other_links) && normalized.other_links.length) normalized.other_links = normalized.other_links;

        console.log("Normal preview fetched normalized row:", normalized);
        setRow(normalized);
      } catch (err: any) {
        setError(String(err));
      } finally {
  ...
  return <NormalPreview data={row} showFooter={false} />;
}