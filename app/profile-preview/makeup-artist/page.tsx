...
import MakeupArtistPreview from "@/app/templates-preview/MakeupArtistPreview";
...
export default function MakeupArtistProfilePreviewPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!listRes.error && Array.isArray(listRes.data)) {
          ...
          if (found) {
            ...
            // Safely extract public URL: support SDK shapes { data: { publicUrl } } and older { publicURL }
    ...
    const fetchRow = async () => {
      ...
      try {
        ...
        const keys = ["avatar","heroImage","portfolio","reviews"];
        keys.forEach(k => {
        ...
        const slug = normalized.slug || "makeup-artist";

        const imageFields = ["avatar","heroImage","portfolio"];
        for (const field of imageFields) {
          if (!normalized[field]) continue;
          if (Array.isArray(normalized[field])) {
            const resolved = await Promise.all(normalized[field].slice(0, 12).map((v: any) => resolveImage(base, slug, field, v)));
        ...
        console.log("Makeup-artist preview fetched normalized row:", normalized);
        setRow(normalized);
      } catch (err: any) {
        setError(String(err));
      } finally {
  ...
  return <MakeupArtistPreview data={row} showFooter={false} />;
}