...
import GymTrainerPreview from "@/app/templates-preview/GymTrainerPreview";
...
export default function GymTrainerProfilePreviewPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // parse lists
        const keys = ["programs","classes","testimonials","avatar","heroImage"];
        keys.forEach(k => {
        ...
        const slug = normalized.slug || "gym-trainer";

        // resolve images
        const imageFields = ["avatar","heroImage"];
        for (const field of imageFields) {
          if (!normalized[field]) continue;
          if (Array.isArray(normalized[field])) {
            const resolved = await Promise.all(normalized[field].slice(0, 2).map((v: any) => resolveImageValue(base, slug, field, v)));
        ...
        console.log("Gym trainer preview fetched normalized row:", normalized);
        setData(normalized);
      } catch (err: any) {
  ...
  // render with showFooter=false so "Use this template" is hidden in contact area
  return <GymTrainerPreview data={data} showFooter={false} />;
}